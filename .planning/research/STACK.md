...
**Domain:** Two-sided rental marketplace web app (farm equipment rental — owners list, farmers book)
**Researched:** 2026-06-26
**Confidence:** MEDIUM (stack choices verified against current npm registry versions + multiple web sources; no Context7/official-docs deep-dive was available this pass, so treat library-specific API details as directionally correct, verify exact syntax against official docs during implementation)

## Verdict on the Pre-Chosen Stack

**The chosen stack holds up. Keep it as-is, with four corrections/additions:**

1. **Pin to Next.js 15, not "latest."** As of this research, Next.js 16 is the `latest` dist-tag on npm (16.2.9) and is now the default for new projects. The project's constraint explicitly says "Next.js 15 App Router" — this is a deliberate, reasonable choice (15 is mature, stable, well-documented, avoids being an early adopter of 16's Turbopack-by-default production builds for a college/portfolio deadline). **Action:** when scaffolding, explicitly use `npx create-next-app@15` or pin `"next": "15.5.x"` in package.json — do not let `create-next-app` (no version pin) silently install 16.

2. **The real fix for "no double-booking" is a Postgres database constraint, not application code.** Server-side validation in a Server Action/API route is necessary but **not sufficient** — under concurrent requests two "available" checks can both pass before either insert commits (classic check-then-act race). The industry-standard fix is a Postgres `EXCLUDE` constraint using `btree_gist` + `daterange`. This must be added to the schema in Phase 1, not deferred. See Architecture/Pitfalls implications below.

3. **react-leaflet needs `next/dynamic` with `ssr: false`**, isolated behind a dedicated `'use client'` map component. This is a known, still-current pattern (confirmed against open GitHub issues as recently as Next.js 15.1.6) — not a one-off bug, but baseline integration requirement. Build this in from the start rather than discovering it mid-Phase.

4. **Use the official `openai` npm package (v6.x) pointed at NVIDIA's `baseURL`**, not a NIM-specific SDK (none exists/is needed) and not the heavier `@ai-sdk/openai-compatible` (that's for projects already using Vercel AI SDK's streaming/tool-call abstractions — overkill for a single FAQ chatbot feature). Simpler dependency surface for an 8-hour-build college project.
...
npx create-next-app@15 agrirent --typescript --tailwind --app --eslint
...
npx shadcn@latest add button card dialog form input select calendar sonner badge avatar dropdown-menu

# Maps
npm install leaflet@^1.9 react-leaflet@^5
...
# Dev dependencies
npm install -D typescript@^5.7 @types/node @types/react @types/react-dom
```
...
**If you outgrow Leaflet's marker-count comfort zone (hundreds+ equipment pins rendered simultaneously):**
...
**If the booking-conflict check needs to span multiple related resources (e.g., equipment bundles, not just a single equipment_id):**
...
**If you later add payments (explicitly deferred to v2 per PROJECT.md):**
...
---
*Stack research for: AgriRent — two-sided farm equipment rental marketplace*
*Researched: 2026-06-26*
